...
foreach($explore_points as $explore_point):
    if (false === isset($explore_point->ID, $explore_point->post_type) || 'explore-character' == $explore_point->post_type || false === get_post($explore_point->ID)) {
        continue;
    }
    ...
    $background_url = true === in_array($explore_point->post_type, ['explore-weapon', 'explore-point', 'explore-character', 'explore-enemy'], true) ? "background: url(" . esc_url($map_url) . ") no-repeat;" : '';
