...
class Menu {
	...
	public function registerGameOptions(): void {
		...
		foreach ( $settings as $key => $value ) {
			register_setting(
				...
				$key,
				array(
					'sanitize_callback' => array( $this, 'sanitizeGameOption' ),
				)
			);
	...
	protected function getGameOptionSettings(): array {
		...
		return array(
			'explore_game_page'                   => array( 'select', 'Page For Game', 'This is the page on your website that users will play your game on.', $pages ),
			'explore_first_area'                  => array( 'select', 'Starting Area', 'The starting area/level of the game.', $areas ),
			'explore_main_character'              => array( 'select', 'Main Character', 'Your main character that users will control first.', $characters ),
			'explore_default_weapon'              => array( 'select', 'Default Weapon', 'The starting weapon your main character will have. (Can be "fist" for no weapon)', $weapons ),
			'explore_require_login'               => array( 'checkbox', 'Require Login', 'Require users to login in order to play or give a "logged out" option.' ),
			'explore_google_login_client_id'      => array( 'text', 'Google Login ClientID', 'Add your Google client id to allow SSO login. (Search google how if you this is confusing)' ),
			'explore_google_tts_api_key'          => array( 'text', 'Google TTS API Key', 'Add your Google TTS API key to allow cutscenes and explainers to talk using text to speech.' ),
			'explore_hud_bars'                    => array( 'multiselect', 'HUD bars', 'Choose which bars you wish to use for your game.', array( 'health', 'mana', 'power', 'money', 'points' ) ),
			'explore_settings_icon'               => array( 'upload', 'Settings Icon', 'Override settings icon in HUD' ),
			'explore_hide_storage'                => array( 'checkbox', 'Hide Storage Menu', 'If checked the storage menu will not appear in HUD.' ),
			'explore_storage_icon'                => array( 'upload', 'Storage Menu Icon', 'Override storage menu icon in HUD' ),
			'explore_crew_icon'                   => array( 'upload', 'Crewmate Menu Icon', 'Override crewmate menu icon in HUD' ),
			'explore_money_image'                 => array( 'upload', 'Money Icon', 'Override money icon for in game currency.' ),
			'explore_indicator_icon'              => array( 'upload', 'Indicator Icon', 'Override your indicator icon that shows when "focus view" or "character" game assets are interactable.' ),
			'explore_arrow_icon'                  => array( 'upload', 'Arrow Icon', 'Override the default arrow icon that is used to point for explainer popups.' ),
			'explore_cutscene_border_color'       => array( 'color', 'Cutscene Border Color', 'The cutscene popup border color.' ),
			'explore_cutscene_border_size'        => array( 'number', 'Cutscene Border Size', 'The border size of the Cutscene popups (in pixel).' ),
			'explore_cutscene_border_radius'      => array( 'number', 'Cutscene Border Radius', 'The border radius of the Cutscene popups.' ),
			'explore_cutscene_border_style'       => array( 'select', 'Cutscene Border Style', 'The border style of the Cutscene popups.', array( 'solid', 'dashed', 'dotted' ) ),
			'explore_skip_button_color'           => array( 'color', 'Skip Button Color', 'The skip button background color (text is white).' ),
			'explore_explainer_border_color'      => array( 'color', 'Explainer Border Color', 'The border color of the explainer popups.' ),
			'explore_explainer_border_size'       => array( 'number', 'Explainer Border Size', 'The border size of the explainer popups (in pixel).' ),
			'explore_explainer_border_radius'     => array( 'number', 'Explainer Border Radius', 'The border radius of the explainer popups.' ),
			'explore_explainer_border_style'      => array( 'select', 'Explainer Border Style', 'The border style of the explainer popups.', array( 'solid', 'dashed', 'dotted' ) ),
			'explore_crewmate_hover_border_color' => array( 'color', 'Playable Character Border Color', 'The border color when you hover over a character in the crew mate selector.' ),
			'explore_intro_video'                 => array( 'upload', 'Intro Video', 'The video that will play when users first visit the game page.' ),
			'explore_start_music'                 => array( 'upload', 'Start Screen Music', 'The music that will play after the intro video and on the start screen.' ),
			'explore_signin_screen'               => array( 'upload', 'Sign In Screen Background Image', 'The image/video that will show on the start screen.' ),
			'explore_walking_sound'               => array( 'upload', 'Walking Sound Effect', 'The sound that will play when your main character walks.' ),
			'explore_points_sound'                => array( 'upload', 'Sound When Points Are Given', 'The sound that will play when you complete a mission or collect something.' ),
		);
	}
	...
	public function sanitizeGameOption( mixed $value ): mixed {
		...
		$choices = $settings[ $option_name ][3] ?? array();
