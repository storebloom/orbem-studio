...
$game_url = get_option('explore_game_url', get_home_url());
...
if ( false !== $explore_area ) {
    $is_area_cutscene = get_post_meta($explore_area[0]->ID, 'explore-is-cutscene', true);
    $explore_area_map = get_post_meta($explore_area[0]->ID, 'explore-map-svg', true);
    $explore_area_start_top = get_post_meta($explore_area[0]->ID, 'explore-start-top', true);
    $explore_area_start_left = get_post_meta($explore_area[0]->ID, 'explore-start-left', true);
}
...
<main id="primary"<?php echo esc_attr(true === $is_admin ? ' data-devmode=true' : ''); ?> class="site-main<?php echo esc_attr($rst); ?>">
	<div class="explore-overlay engage" style="background: url(<?php echo esc_attr($signin_screen); ?>) no-repeat center;background-size: cover;height: 100svh;left: 0;position: fixed;top: 0;width: 100%; z-index: 10;">
			...
			<?php if (is_user_logged_in()) : ?>
				<div class="character-choice">
                    <div class="character-item">
                        <img src="<?php echo $plugin_dir . '/assets/src/images/graeme-avatar.gif'; ?>" />
                        <br>
                        <small>
	...
	<div class="container <?php echo esc_attr($location); ?>" style="background: url(<?php echo esc_url($explore_area_map); ?>) no-repeat left top; background-size: cover;">
		...
		<div class="default-map" data-iscutscene="<?php echo esc_attr($is_area_cutscene); ?>" data-startleft="<?php echo false === empty($explore_area_start_left) ? esc_attr($explore_area_start_left) : ''; ?>" data-starttop="<?php echo false === empty($explore_area_start_top) ? esc_attr($explore_area_start_top) : ''; ?>">
            <?php if (false !== $explore_area): ?>
                ...
                <?php echo html_entity_decode(Explore::getMapItemHTML($explore_points, get_current_user_id(), $explore_area[0]->post_name)); ?>
