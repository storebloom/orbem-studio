...
<div class="mission-list">
    <?php foreach($explore_missions as $mission) :
        if (false === $mission instanceof WP_Post) {
            continue;
        }

        $next_mission                        = get_post_meta($mission->ID, 'explore-next-mission', true);
        $linked_mission[$mission->post_name] = false === empty($next_mission) ? $next_mission : '';
        $parent_mission                      = '';
        ...
        if (false === in_array($mission->post_name, $completed_missions, true)) :
            ...
            $mission_points             = get_post_meta($mission->ID, 'explore-value', true);
            ...
            <div
                ...
                data-nextmission="<?php echo esc_attr(implode(',', (is_array($next_mission) ? array_keys($next_mission) : [$next_mission]))); ?>"
                data-hazardremove="<?php echo esc_attr($hazard_remove) ?? ''; ?>"
