...
export function initImageUpload() {
	...
	const removeImageButtons = document.querySelectorAll(
		'.remove_image_button'
	);
...
function initExploreUploadButton( button ) {
	button.addEventListener( 'click', function ( e ) {
		...
		imageUploader.on( 'select', function () {
			...
			let imageUploadPoint = document.getElementById(
				`explore-character-images[${ direction }]`
			);
			if ( imageUploadPoint && 'image' === attachment.type ) {
				...
				if ( imageUploadParent ) {
					imageUploadPoint = imageUploadParent.querySelector(
						'.explore-upload-field'
					);
				}
			}
			...
			// product video
			const videoUploadPoint =
				document.getElementById( '_product_video_url' );
			...
			const videoUploadThumbnail = document.getElementById(
				'_product_video_url_thumbnail'
			);
...
function initExploreRemoveButton( button ) {
	button.addEventListener( 'click', function ( e ) {
		...
		// Get the direction from the button's data attribute
		const direction = this.dataset.direction;
		let characterValue = document.getElementById(
			`explore-character-images[${ direction }]`
		);
		...
		if ( characterValue ) {
			...
			if ( imageUploadParent ) {
				characterValue = imageUploadParent.querySelector(
					'.explore-upload-field'
				);
			}
		}
