...
export function engageDevMode() {
	...
	function zoomGameContainer( whichWay ) {
		...
		if ( gameContainer ) {
			let currentZoom =
				undefined !== gameContainer.style.transform &&
				'' !== gameContainer.style.transform
					? gameContainer.style.transform
					: '1';
			currentZoom = parseFloat(
				currentZoom.replace( 'scale(', '' ).replace( ')', '' )
			);
			currentZoom =
				'in' === whichWay
					? +( currentZoom + 0.1 ).toFixed( 10 )
					: +( currentZoom - 0.1 ).toFixed( 10 );
	...
	setTimeout( () => {
		...
		const pinPointContainer = document.querySelector(
			'.pinpoint-container'
		);
			...
			addNewListItems.forEach( function ( item ) {
				item.addEventListener( 'click', () => {
					...
					fetch( filehref, {
						...
						.then( ( data ) => {
							const addNewFields =
								document.querySelector( '.add-new-fields' );
		...
		if ( findItems && findItems.length > 0 ) {
			findItems.forEach( ( item ) => {
				...
				if ( editButton && showItem ) {
					const findContainer =
						editButton.closest( '.find-explore-item' );
					...
					editButton.addEventListener( 'click', () => {
						if (
							true !== editButton.classList.contains( 'created' )
						) {
							const sizeContainer =
							...
							heightInput.value =
								definedHeight && '' !== definedHeight
									? definedHeight
									: theNewSizeItem.dataset?.height;
							widthInput.type = 'number';
							widthInput.value =
								definedWidth && '' !== definedWidth
									? definedWidth
									: theNewSizeItem.dataset?.width;
							...
							if (
								'explore-character' === item.dataset.posttype ||
								'explore-enemy' === item.dataset.posttype
							) {
								const recordPathLabel =
									document.createElement( 'label' );
								const recordPathInput =
									document.createElement( 'input' );
								recordPathInput.type = 'checkbox';
								recordPathLabel.textContent =
									'Record Walking Path';
								...
								recordPathInput.addEventListener(
									'change',
									() => {
				...
				itemTitle.addEventListener( 'click', ( e ) => {
					const theFinderItem =
						e.target.closest( '.find-explore-item' );
					const theItem = document.querySelector(
						'.' +
							theFinderItem.dataset.class +
							'[data-genre="' +
							theFinderItem.dataset.posttype +
							'"]'
					);
					const currentSelected =
						document.querySelector( '.map-item.selected' );
					const currentListSelected = document.querySelector(
						'.find-explore-item.selected'
					);
					...
					if ( theItem ) {
						mainCharacter.style.left =
							parseInt( theItem.style.left.replace( 'px', '' ) ) -
							200 +
							'px';
						mainCharacter.style.top =
							parseInt( theItem.style.top.replace( 'px', '' ) ) -
							200 +
							'px';
			...
			const finderListContainers = document.querySelectorAll(
				'.explore-list-group'
			);
		...
		const engageWallBuilder =
			document.getElementById( 'engage-wallbuilder' );
			...
			function handleWallDragStart( event ) {
				...
				function handleWallMouseMove( event ) {
					...
					if ( wallElement ) {
						...
						const mouseX =
							( event.clientX - mapRect.left ) / devZoom;
						const mouseY =
							( event.clientY - mapRect.top ) / devZoom;
...
function makeNewFormSub() {
	...
	if ( submitNewItem ) {
		submitNewItem.addEventListener( 'submit', function ( e ) {
			...
			const filehref = `${ OrbemOrder.siteRESTURL }/add-new/`;
			const selectedPostType = document.querySelector(
				'#add-new-list li.engage'
			);
			let postType = '';
