...
class Explore {
	...
	public function createApiPostsMetaField(): void {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'handleGoogleOauthCallback' ),
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'addCharacterPoints' ),
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'saveStorageItem' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'getOrbemArea' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'getItemDescription' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'saveCoordinates' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'resetExplore' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'equipNewItem' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'saveMission' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			$namespace,
			'/save-materialized-item/',
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'saveMaterializedItem' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'addCharacter' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'enableAbility' ),
				'permission_callback' => $permission_callback,
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'setPreviousCutsceneArea' ),
				'permission_callback' => $permission_callback,
			)
		);
	}
	...
	public function addSpell( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( 0 < $spell_id ) {
			...
			$explore_magic  = false === empty( $explore_magic ) ? $explore_magic : array(
				'defense' => array(),
				'offense' => array(),
			);
			...
			if ( false === is_wp_error( $spell_type ) && true === is_array( $spell_type ) ) {
				foreach ( $spell_type as $type ) {
					if ( true === in_array( $type->slug, array( 'defense', 'offense' ), true ) ) {
	...
	public function savePoint( $userid, $type, $amount, $item, bool $reset = false ): void {
		if ( 0 < $userid && false === empty( $item ) ) {
			...
			$explore_points         = false === empty( $current_explore_points ) && is_array( $current_explore_points ) ? $current_explore_points : array(
				'health'  => array(
					'points'    => 100,
					'positions' => array(),
				),
				...
				'gear'    => array( 'positions' => array() ),
				'weapons' => array( 'positions' => array() ),
			);

			if ( 'communicate' === $type ) {
				$current_communicates = get_user_meta( $userid, 'explore_received_communicates', true );
				$current_communicates = is_array( $current_communicates ) ? $current_communicates : array();
			...
			if ( true === is_array( $item ) ) {
				if ( ! isset( $explore_points[ $type ]['positions'] ) || ! is_array( $explore_points[ $type ]['positions'] ) ) {
					$explore_points[ $type ]['positions'] = array();
				}
	...
	public function saveDrag( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( false === empty( $current_explore_drag ) ) {
			$current_explore_drag[ $slug ] = array(
				'top'  => $top,
				'left' => $left,
			);
		} else {
			$current_explore_drag = array(
				$slug => array(
					'top'  => $top,
					'left' => $left,
				),
	...
	public function saveMaterializedItem( \WP_REST_Request $request ): \WP_REST_Response {
		...
		$current_materialized_items = false === empty( $current_materialized_items ) ? $current_materialized_items : array();
		$final_items                = array();
	...
	public function enableAbility( \WP_REST_Request $request ): \WP_REST_Response {
		...
		$current_abilities = false === empty( $current_abilities ) ? $current_abilities : array();
	...
	public function saveEnemy( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( false === empty( $explore_enemies ) ) {
			...
			$explore_enemies = array( $position );
		}
	...
	public function saveMission( \WP_REST_Request $request ): \WP_REST_Response {
		...
		$explore_missions = is_array( $explore_missions ) ? $explore_missions : array();
		...
		if ( false === empty( $explore_missions ) ) {
			...
			$explore_missions = array( $mission );
		}
	...
	public function equipNewItem( \WP_REST_Request $request ): \WP_REST_Response {
		...
		$current_equipped = false === empty( $current_equipped ) ? $current_equipped : array();
		$the_effect_type  = '';
		...
		if ( true === in_array( $effect_type, array( 'mana', 'health', 'power' ), true ) ) {
			...
			if ( true === is_array( $current_equipped[ $the_effect_type ] ) ) {
				foreach ( $current_equipped[ $the_effect_type ] as $current_array ) {
					if ( false === in_array( $item_id, array_keys( $current_array ), true ) ) {
						$current_equipped[ $the_effect_type ][] = array( $item_id => $amount );
					}
				}
			}
	...
	public function saveStorageItem( \WP_REST_Request $request ): \WP_REST_Response {
		...
		$current_storage_items = false === empty( $current_storage_items ) ? $current_storage_items : array();
		$item_subtype          = get_post_meta( $id, 'explore-value-type', true );
		$subtype               = '';
		$menu_map_array        = $current_storage_items[ $menu_map ] ?? array();
		...
		if ( true === $remove ) {
			...
			$has_dupe             = false;
			$default_weapon_array = false !== $default_weapon_obj_id ? array(
				'name' => $default_weapon,
				'type' => 'weapons',
				'id'   => $default_weapon_obj_id,
			) : array();

			if ( true === empty( $current_storage_items ) ) {
				$menu_map_array = array(
					'items'   => array(),
					'weapons' => array( $default_weapon_array ),
					'gear'    => array(),
				);
			} else {
	...
	public function getItemDescription( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( true === in_array( $type, array( 'mana', 'health', 'power' ), true ) ) {
			$item_type = $type;
		}
		...
		if ( false === empty( $gear_equipped[ $item_type ] ) && true === is_array( $gear_equipped[ $item_type ] ) ) {
			foreach ( $gear_equipped[ $item_type ] as $current_array ) {
				if ( true === in_array( $item, array_keys( $current_array ), true ) ) {
					...
					$content = str_replace(
						array( 'Equip', 'equip', 'Ununequip' ),
						array( 'Unequip', 'unequip', 'Unequip' ),
						$content
					);
				}
			}
	...
	public static function getMapSVG( WP_Post $explore_area ): false|string {
		...
		$response = wp_remote_get( $map_url, array( 'timeout' => 10 ) );
	...
	public static function getExplorePoints( $position ): array {
		...
		if ( $position || $first_area ) {
			$args = array(
				'posts_per_page' => -1,
				'post_type'      => array( 'explore-weapon', 'explore-area', 'explore-point', 'explore-character', 'explore-enemy', 'explore-sign', 'explore-wall' ),
	...
	public static function getMapItemHTML( array $explore_points, string $current_location ): string {
		...
		$html               = '';
		$dead_ones          = false === empty( $dead_ones ) ? $dead_ones : array();
		$health             = '';
		$explore_enemy_type = '';
		...
		if ( false === empty( $explore_points ) ) {
			foreach ( $explore_points as $explore_point ) {
				...
				if (
					...
					// Raw content for game engine; do not apply WordPress filters.
					$html .= true === in_array( $explore_point->post_type, array( 'explore-character', 'explore-sign' ), true ) ? wp_kses_post( $explore_point->post_content ) : '';

					if ( true === in_array( $explore_point->post_type, array( 'explore-character', 'explore-enemy' ), true ) ) {
					...
					if ( 'explore-enemy' === $explore_point->post_type && false === empty( $boss_waves ) ) {
						if ( true === $pulse_wave ) {
							...
							$html .= self::getSVGCode(
								plugin_dir_url( __DIR__ ) . 'assets/src/images/pulse.svg'
							);
							$html .= '</div>';
						}
					}
					...
					if ( true === in_array( $explore_point->post_type, array( 'explore-enemy', 'explore-character' ), true ) && false === in_array( '', array( $path_trigger_width, $path_trigger_height ), true ) ) {
	...
	public static function getMapCutsceneHTML( array $explore_cutscenes, string $position, int $userid ): string {
		...
		foreach ( $explore_cutscenes as $explore_cutscene ) {
			...
			// Trigger Cutscene.
			if ( false === in_array( '0', array( $path_trigger_width, $path_trigger_height ), true ) && false === $is_cutscene_triggered ) {
		...
		if ( false === empty( $communication_background ) ) {
			...
			$included_posts = get_posts(
				array(
					...
					) ? $current_received[ $communication_type->term_id ] : array(),
					'post_status'    => 'publish',
					'no_found_rows'  => true,
				)
			);

			$explore_communicates = is_array( $explore_communicates ) ? $explore_communicates : array();
			$included_posts       = is_array( $included_posts ) ? $included_posts : array();
			...
			foreach ( $explore_communicates as $explore_communicate ) {
				$explore_communicate_meta = array();
	...
	public static function getExplainerHTML( array $explore_explainers, string $type ): string {
		...
		foreach ( $explore_explainers as $explainer ) {
			...
			if ( $type === $explainer_type ) {
				...
				$arrow_style                = is_array( $arrow_style ) ? $arrow_style : array();
	...
	public static function getMapAbilitiesHTML( array $explore_abilities ): string {
		...
		$user   = wp_get_current_user();
		$userid = $user->ID;
		$magics = 0 !== $userid ? get_user_meta( $userid, 'explore_magic', true ) : array();
	...
	public function registerPostType(): void {
		...
		$taxo_types = array(
			'explore-communication-type' => array(
				'name'       => 'Communication Type',
				'post-types' => array( 'explore-communicate' ),
			),
		);
	...
	public static function getCharacterImages( string|\WP_Post $main_character ): array {
		...
		if ( is_string( $main_character ) ) {
			$posts = get_posts(
				array(
					'post_type'      => array( 'explore-character', 'explore-enemy' ),
		...
		$images        = $meta['explore-character-images'] ?? array();
		$weapon_images = $meta['explore-weapon-images'] ?? array();
